...
from omni.ui import VStack, Label
import omni.kit.notification_manager as notification

from .utils import count_avatars_inside

# # Display a notification message
# notification.post_notification("This is a notification message!")
...
class OmniverseJsonParseExtension(omni.ext.IExt):
    ...
    def setup_window(self):
        ...
        def toggle_play_stop():
            ...
            if play_stop_label.as_string == "Play":
                ...
                avatars_inside = count_avatars_inside(avatars)

                # time.sleep(10)

                notification.post_notification(
                    f"Avatars inside the room (Mesh7342): {avatars_inside[0]}\n"
                    f"Avatars inside the room (Mesh7332): {avatars_inside[1]}",
                ...
                with ui.VStack():
                    ...
                    def on_json():
                        self._spawn_characters_form_json(self.json_path)
                            ...
                            # with VStack():
                            #     Label("Message on the Screen! - mesh7342, 7332")
                        ...
                        # # Helper function to check if a position is within the bounding box
                        # def is_within_bounding_box(position):
                        #     x, y, z = position
                        #     return (bounding_box['min_x'] <= x <= bounding_box['max_x'] and
                        #             bounding_box['min_y'] <= y <= bounding_box['max_y'] and
                        #             bounding_box['min_z'] <= z <= bounding_box['max_z'])

                        # # Check avatars' actions for overlapping positions
                        # time_slots = {}
                        # avatars = self.get_json_manager().read_from_json(self.json_path)

                        # for avatar_name, details in avatars.items():
                        #     for action in details['actions']:
                        #         finish_pos = action.get('finish_pos')
                        #         if finish_pos and finish_pos != '_':
                        #             # Split and convert finish position to float
                        #             finish_x, finish_y, finish_z, *_ = map(float, finish_pos.split())
                        #             finish_position = (finish_x, finish_y, finish_z)

                        #             # Calculate the time this avatar will be at the finish position
                        #             finish_time = action.get('finish_time')
                        #             start_time = action.get('start_time')

                        #             # If times are null, set them to a default (for simulation purposes)
                        #             if finish_time is None:
                        #                 finish_time = start_time + 5  # Assume it takes 5 seconds to reach

                        #             if start_time is None:
                        #                 start_time = finish_time - 5  # Assume it takes 5 seconds to reach

                        #             # Record the time slot for this finish position
                        #             if is_within_bounding_box(finish_position):
                        #                 time_slots.setdefault((finish_x, finish_y), []).append((avatar_name, start_time, finish_time))

                        # # Find overlapping avatars in the same time slot
                        # overlapping_avatars = []
                        # for position, slots in time_slots.items():
                        #     for i in range(len(slots)):
                        #         for j in range(i + 1, len(slots)):
                        #             avatar_1, start_1, finish_1 = slots[i]
                        #             avatar_2, start_2, finish_2 = slots[j]
                        #             if (start_1 <= finish_2 and start_2 <= finish_1):  # Check for overlap
                        #                 overlapping_avatars.append((avatar_1, avatar_2))

                        # print("Overlapping avatars:", overlapping_avatars)



                    with ui.HStack(width=449):
                        ui.Button("Load avatars from json file", clicked_fn=on_json)
                        ...
                        #     print("stoppped")
                        #     omni.timeline.get_timeline_interface().play()
                        #     if play_stop_label.as_string == "Play" and omni.timeline.get_timeline_interface().is_playing:
                        #         omni.timeline.get_timeline_interface().pause()
    ...
    def _spawn_characters_form_json(self, json_path):
        ...
        print("chilld", character_root_prim.GetChildren())

        # stage = omni.usd.get_context().get_stage()
        # ava = stage.GetPrimAtPath('/World/Characters/avatar_1')
        # if ava:
        #     avatar = ArticulationView('/World/Characters/avatar_1')
        #     if avatar:

        #         poss = avatar.get_joint_positions()
        #         print("avatarrrr", poss)
    ...
    def _setup_json_characters(self):
        ...
        for prim in self.stage.Traverse():
            if prim.GetTypeName() == "SkelRoot" and UsdGeom.Imageable(prim).ComputeVisibility() != UsdGeom.Tokens.invisible:
                ...
                # avatar = ArticulationView(prim.GetPath().pathString)
                # if avatar:
                #     print(f"Setting posture for {prim.GetPath().pathString}")

                #     # Example: Set joint positions for a specific posture
                #     # You can adjust the values depending on the skeleton structure
                #     joint_positions = [10, 10, 10]  # Adjust with valid joint values
                #     avatar.set_joint_positions(joint_positions)

                # avat = ag.get_character(str(prim.GetPath().pathString))
                # avat.set_variable("Action", "Sit")
